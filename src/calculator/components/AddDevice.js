    ...
    render() {
      return (
        <div className='row-btn-line'>
            <input className='input_name' placeholder='Название' onChange={(e) => {this.setState({name: e.target.value})}}/>
            <div className='grid_power_input'>
                <input className='input_power' placeholder='Мощность' onChange={(e) => {this.setState({power: e.target.value})}}/>
                <p className='Vatts_text'>Вт</p>
            </div>
          <button className="plus-line-btn"
          onClick={() => {
            this.props.onAdd(this.state.deviceAdd)
          }}>
